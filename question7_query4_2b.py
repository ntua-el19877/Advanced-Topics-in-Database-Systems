...
for hint in join_hints:
    df,spark=dataframe_setup.session_setup(4,"Query4_2b_"+hint)
    ...
    print(f"Applying join hint {hint}")
    if hint == "broadcast":
            crimes_with_closest_prec_distance=crimes_with_closest_prec\
                .join(broadcast(police_stations),
                    crimes_with_closest_prec["Prec code"]==police_stations["PREC"])
    else:
            crimes_with_closest_prec_distance = crimes_with_closest_prec\
                .join(police_stations.hint(hint), 
                    crimes_with_closest_prec["Prec code"] == police_stations["PREC"])
    crimes_with_closest_prec_distance.explain()
    crimes_with_closest_prec_distance = crimes_with_closest_prec_distance\
